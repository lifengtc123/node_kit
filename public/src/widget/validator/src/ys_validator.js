define(function(require,exports,module){
    ...
    jQuery.validator.addMethod('noSamevalue', function (value, element) {
        // var value = value[0].replace(/\_?/g,'\\_');
        // 上文注释中替换'_'为'\_'的代码有问题，修改为如下
        var temp = value[0].replace(/\_/g,'\\_');
        return this.optional(element) || !(new RegExp('^' + temp + '+$').test(value));
    }, '不能为同一符号');
    ...
    jQuery.validator.addMethod('deviceNameControl', function (value, element) {
        return this.optional(element) || /^[a-z][a-z0-9\_]+[a-z0-9]$/.test(value);
    }, '只能输入数字、小写字母和“-”字符,以小写字母开始,但不能已“-”字符结尾');
    jQuery.validator.addMethod('deviceSerialControl', function (value, element) {
    ...
    jQuery.validator.setDefaults({
        ...
        success: function (label, element) {
           ...
           // $.trim($ele.val()).length && ($ele.next('.success').length || $ele.after('<span style="position:absolute;left: ' + (Number(pos.left) + $ele.outerWidth() + margin_left + 4) + 'px;top:' + (Number(pos.top) + margin_top  + $ele.outerHeight() - 17 - padding_bottom) + 'px" class="success"></span>'));
            $.trim($ele.val()).length && ($ele.next('.success').length || $ele.after('<span style="display:inline-block;" class="success"></span>'));
        }
    });
});