...
define(function (require, exports, module) {
    ...
    var dftCfg = {
        ...
        msgCls: 'ys-content',	//消息框内容的class名称，用于自定义样式，默认为ys-content,仅在iframe:false时有效
        ...
        noHeight: false,          // 不固定高度
        fnQueue:[],               // 弹出框弹出后，执行函数队列
        isOnly:false,             // 弹出框关闭后，不会弹出队列里弹出框
        autoPull:true             // 关闭弹出框后，自动从队列中删除
    }, curCfg = {};
    (function () {
        ...
        var container = [
            ...
            "<div class='ysDialog_close' title='关闭'><strong>&times;</strong></div>",
            "</div></div></div></div>",
        ...
        var cacheWin = []; //队列中的窗口
        var winVisible = function (visible,sign) {
            var fn = visible === false ? 'off' : 'on';
            $(window)[fn]('scroll', curCfg.fixPosition && !useFixed ? scrollEvent : saveWinInfo);
            $(ys_wins).css('position', curCfg.fixPosition && useFixed ? 'fixed' : 'absolute').css('width','100%').css('height','100%');
            $(document)[fn]('keyup', keydownEvent); // IE8下 keyup事件无法冒泡到window对象
            //$(window)[fn]('keyup', keydownEvent);
            if (visible === false) { //关闭
                ...
                var closeFn = function () {
                    ...
                    if(curCfg.autoPull){
                        cacheWin.shift();
                        curCfg.closeHandler && curCfg.closeHandler(sign);
                        if (!curCfg.isOnly && cacheWin.length) ysDialog.show.apply(null, cacheWin[0].concat(true, true))
                    } else{
                        $(ysDialog.getContent()).find('input,textarea').each(function () {
                            var $this = $(this);
                            if($this.is(':checkbox')) {
                        ...
                        cacheWin[0] && (cacheWin[0][0][0].content = $(ysDialog.getContent()).html());
                        curCfg.closeHandler && curCfg.closeHandler(sign);
                    }
                };
                /*渐变方式关闭*/
        ...
        var destroy = function (btn) {
            !curCfg.useSlide && maskVisible(false);
            winVisible(false,btn);
        }; //取得iframe
        ...
        $.extend(ysDialog, {
            ...
            getPage: getPage,
            isShow:function(){
                return $(ys_win).css('display') != 'none';
            },
            getContent: function () {
                return ys_body.firstChild || '';
            },
            delNextWin:function(index){
                if(index == null){
                    index = 1;
                }
                cacheWin.splice(index,1);
            },
            nextBind:function(index,callback){
                if(typeof index != 'number'){
                    callback = index;
                    index = 1;
                }

                var winArg = cacheWin[index];
                if(winArg){
                    winArg[0][0].fnQueue.push(callback);
                }
            },
            ...
            show: function (args, fargs, show, inShow) {
                ...
                o.fnQueue = o.fnQueue || [];
                !o.contentBind && (o.contentBind = function(){});
                !o.closeHandler && (o.closeHandler = function(){});
                (o.autoPull == null) && (o.autoPull = true);
                if (!show && cacheWin.push([[o], fargs]) && cacheWin.length > 1) return;
                /*支持两种参数传入方式:(1)JSON方式 (2)多个参数传入*/
                if (!inShow && show) {
                    if(cacheWin.length && this.isShow()){
                ...
                init();
                curCfg.fnQueue.push(curCfg.contentBind);
                var fn;
                while(fn = curCfg.fnQueue.shift()) fn();
            },
    ...
    $.extend(ysDialog, {
        ...
        warn: function () {
            ...
            if (!$.isPlainObject(arguments[0])) {
                conf[0] = {};
                conf[0].content = arguments[0];
                conf[0].closeHandler = arguments[1];
            } else {
                conf = arguments;
            }
            ysDialog.show(conf, {
