/*
	...
	*/
	...
	function currentTime(){
		...
		$el.find(".details .big").html(monthNames[currentDate.getMonth()] + " " + currentDate.getDate() + " 日, " + currentDate.getFullYear());
...
if($('.flot-line').length > 0){
	$(function () {
        ...
        socket.on("cpudata",function(data){
            ...
            var res = [];
            for (var i = 0; i < data.menory.length; ++i)
                res.push([i, data.menory[i]])
            plot2.setData([ {
                label: "menory at %",
                data: res,
                lines: {show: true, fill:true},
