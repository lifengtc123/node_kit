...
boxControllers.controller('MesCtrl', ['$scope', '$state', '$timeout', '$http', '$translate', 'socket',
    function ($scope, $state, $timeout, $http, $translate, socket) {
        ...
        var liheight = $state.height- $(".header").height() - $(".nav").height() - 25;
                            ...
                            if (data != null && data != "") {
                                for (var i = 0; i < data.length; i++) {
                                    ...
                                    if (data[i].type == 0) {
                                        ...
                                        fromtitle = $translate.instant('formTitle');
                                        var filepath= $state.server + "/vv-box/" + data[i].boxNumber + "/" + data[i].filepath.replace("amr","mp3");
                                        
                                        html = html + '<div class="info_box">';
                                        html = html + '<div class="iconfont icon talk">&#xe61d;</div>';
                                        html = html + '<div class="tit"><strong>' + title + '</strong><strong style="float: right;">' + fromtitle + '<a href="#/Info/' + data[i].boxNumber + '">' + data[i].boxName + '</a></strong></div>';
                                        html = html + '<div class="tit"><div class="voice openfile" onclick=openfile2(\"' + data[i].boxNumber + '\",\"' +filepath+ '\",\"' + data[i].time + '\",\"' + data[i]._id + '\")><i></i>';
    ...
    function ($scope, $state, $http, $stateParams, $translate, socket, $timeout) {
        ...
        if ($state.user) {
            ...
            $http.post($state.server + "/appapi/nobilitys/getUsername", obj).success(function (udata) {
                ...
                if (boxNumbers==null||boxNumbers.length<=0) {
                     $state.go("addbox");
                 } else {
                    var nboxid = "";
                     var nboxid2 = "";
                     ...
                     if (udata.lastboxid != "" || udata.lastboxid != null) {
                         for(var i=0;i<udata.boxIds.length;i++){
                             if(udata.lastboxid==boxNumbers[i]){
                         ...
                         if(!ylast){
                             nboxid=udata.boxIds[0];
                         }
                     } else {
                         nboxid = udata.boxIds[0];
                     }
                     boxNumber = nboxid;
                     if(localStorage.getItem("alert")!=""&&localStorage.getItem("alert")!=null&&localStorage.getItem("alert")!="undefined"){
                         var alertm=JSON.parse(localStorage.getItem("alert"));
                         if(alertm.boxname==nboxid){
                             $state.boxlists = alertm.boxlists;
                             $scope.boxname = alertm.boxname;
                             $scope.number = alertm.boxname;
                             $state.alertmessages=alertm.alertmessages;
                             for (var i = 0; i < $state.alertmessages.length; i++) {
                                 $state[$state.alertmessages[i]._id] = "images/microphone.png";
                             }
                         }
                     }
                     $scope.boxname = nboxid;
                     $scope.number =nboxid;
                     //获取所有盒子的信息
                     ...
                     function getOneBox(boxid) {
                         $http.post($state.server + "/appapi/boxes/getOne", {boxid: boxid}).success(function (data) {
                            if(data!="null"){
                                ...
                                $state.aimg = data.image;
                                console.log($state.aimg);
                                $scope.number = data.number;
                                getMessages(data.number);
                            }else{
                                $state.aimg =$state.boxlists[0].image;
                                nboxid2= $state.boxlists[0].number;
                                getMessages(nboxid2);
                            }
                        });
                     }
            ...
            var openindex = "";
             var my_media;
            $scope.openfile = function ($event,filepath, time, index) {
                $("#openmediv").val(1);
                $('.swipe-delete div.open').animate({left: 0}, 200);
                $('.swipe-delete div.open').css('left', '0px').removeClass('open');
                if (!my_media) {
                    $state[index] = "images/microphone.gif";
                    captureSuccess(filepath, time, index);
                } else {
                    my_media.pause();
                    $state[index] = "images/microphone.png";
                    my_media = null;
                    $("#openmediv").val(0);
                }
                function captureSuccess(filepath, time, index){
                    my_media=null;
                    $("#yyMusic").attr("src","http://www.officeshare.cn:1813"+ "/vv-box/" + boxNumber + "/" + filepath);
                    my_media = document.getElementById("yyMusic");
                    my_media.play();
                    if (time != null && time != "") {
                        $timeout(function () {
                        my_media.pause();
    ...
    function ($scope, $state, $stateParams, $timeout, $http, $translate) {
        ...
        if ($state.user) {
            ...
            $scope.openfile = function ($event, filepath, time) {
                ...
                function captureSuccess(filepath, time, index){
                    my_media=null;
                    $("#deMusic").attr("src","http://www.officeshare.cn:1813"+ "/vv-box/" + $scope.data.boxNumber + "/" + filepath);
                    my_media = document.getElementById("deMusic");
                    my_media.play();
                    if (time != null && time != "") {
                        $timeout(function () {
                            $scope.voiceimg = "images/info_voice.png";
...
boxControllers.controller('WatchInfo', ['$scope', '$state', '$http', '$stateParams', '$translate',
    function ($scope, $state, $http, $stateParams, $translate) {
         $scope.closeDetail4= function () {
            $state.oinfo=false;
            $state.openInfo4=false;
        };
